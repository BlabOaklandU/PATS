...
while read f; do \
std=$PoPipe_stdDir/$sname
$PoPipe_srcBaseDir/externalSoftware/muscle/muscle3.8.31_i86linux64 -in $PoPipe_srcBaseDir/orthoGroupsFasta/$f -fastaout $PoPipe_srcBaseDir/muscleFasta/$f 1>$std.out 2>$std.err;
res=$?; if [ $res -ne 0 ]; then polog "$sname error $res"; exit $res; fi; 
done
polog "$sname end";
